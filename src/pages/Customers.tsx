...
export function Customers() {
  ...
  const handleEditCustomer = (updatedData: Customer) => {
    setCustomerList(customerList.map(c => c.id === updatedData.id ? updatedData : c));
  };

  const handleAddCustomer = (customer: Partial<Customer>) => {
    setCustomerList([...customerList, customer as Customer]);
  };
  ...
  const toggleCustomerStatus = (customerId: string) => {
    setCustomerList(customerList.map(c => 
      c.id === customerId 
        ? { ...c, status: c.status === 'active' ? 'inactive' as 'inactive' : 'active' as 'active' }
        : c
    ));
  };
